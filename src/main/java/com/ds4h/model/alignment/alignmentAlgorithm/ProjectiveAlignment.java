...
public class ProjectiveAlignment implements AlignmentAlgorithm{
    ...
    @Override
    public Optional<AlignedImage> align(ImagePoints targetImage, final ImagePoints imageToShift, final ImageProcessor ip) throws IllegalArgumentException {
        if(targetImage.getPoints().length >= LOWER_BOUND && imageToShift.getPoints().length >= LOWER_BOUND){
            ...
            final Optional<ImagePlus> finalImage = Optional.of(MatImagePlusConverter.convert(alignedImage, imageToShift.getName(), ip));
