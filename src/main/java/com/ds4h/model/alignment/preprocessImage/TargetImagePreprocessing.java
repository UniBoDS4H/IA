...
public class TargetImagePreprocessing {
    ...
    private Mat automaticProcess(final ImagePoints targetImage,
    ...
    public Pair<ImagePoints, Map<ImagePoints, MatOfPoint2f>> processImage(final ImagePoints targetImage, final List<ImagePoints> imagesToAlign, final AbstractAutomaticAlignment algorithm) throws IllegalArgumentException{
        ...
        final Optional<ImagePlus> imagePlus = ImagingConversion.fromMatToImagePlus(finalTargetMat,targetImage.getFile().getName());
        ...
        return new Pair<>(result, this.map);
    }
}
